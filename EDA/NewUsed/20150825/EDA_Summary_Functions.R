#-------------------------------------------------------------------------------
# Function for generating summary of the numeric variables required for HTML report creation
...
corAnalysis <- function(x, Data, cutOff=0.8) {
  numVars <- setdiff(names(Data)[which(sapply(Data, class) == "numeric")], x)
  tempCorr <- cor(Data[, x], Data[, numVars], use="pairwise.complete.obs")[1,]
