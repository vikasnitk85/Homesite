...
#-------------------------------------------------------------------------------
paramList <- expand.grid(eta=c(0.02), max_depth=c(7), min_child_weight=c(5, 10, 20),
...
for(i in 1:nrow(paramList)) {
  ...
  write.csv(paramList, "model/xgboost_cv/xgboost_cv_aws_eta_7.csv", row.names=FALSE)
}
