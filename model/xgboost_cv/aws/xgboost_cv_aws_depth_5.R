...
#-------------------------------------------------------------------------------
paramList <- expand.grid(eta=c(0.02), max_depth=c(5), min_child_weight=c(5, 10, 20),
...
for(i in 22:nrow(paramList)) {
  ...
  write.csv(paramList, "model/xgboost_cv/xgboost_cv_aws_eta_5.csv", row.names=FALSE)
}
