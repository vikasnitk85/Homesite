...
#-------------------------------------------------------------------------------
paramList <- expand.grid(eta=c(0.03, 0.02, 0.01), max_depth=c(6, 8, 10, 12), min_child_weight=c(1, 5, 10),
  subsample=c(0.5, 0.7, 0.9), colsample_bytree=c(0.5, 0.7, 0.9))
paramList <- data.table(paramList)

for(i in 50:nrow(paramList)) {
  startTime <- Sys.time()
  ...
  #-------------------------------------------------------------------------------
  set.seed(1234)
  system.time(xgb_cv0 <- xgb.cv(params=param0, data=dtrain, nrounds=1000, metrics=list("auc"), folds=index, verbose=FALSE))
...
write.csv(paramList, "model/xgboost_cv/xgboost_cv_paramList.csv", row.names=FALSE)
