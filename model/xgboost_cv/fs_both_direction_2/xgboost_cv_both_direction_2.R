...
selVars <- c("PropertyField37", "PersonalField10A", "SalesField5", "Field7", "PersonalField1", "SalesField1B",
  "PersonalField27", "PersonalField2", "PersonalField12")
# Version <- 1
...
for(Version in 7:35) {
  ...
  forward_out <- foreach(i=1:length(new_features), .inorder=FALSE, .packages=c("xgboost", "data.table", "caTools", "tcltk", "foreach")) %dopar% {
