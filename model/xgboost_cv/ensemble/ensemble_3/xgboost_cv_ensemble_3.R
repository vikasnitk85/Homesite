...
# library(RStudioAMI)
# linkDropbox()
pkgs <- c("caret", "data.table", "xgboost", "caTools", "doSNOW", "R.utils")
...
#-------------------------------------------------------------------------------
paramList <- expand.grid(eta=c(0.1), max_depth=c(5:8), min_child_weight=c(5, 10, 15),
...
paramList[max_depth==5, nrounds:=350]
paramList[max_depth==6, nrounds:=250]
paramList[max_depth==7, nrounds:=200]
paramList[max_depth==8, nrounds:=150]
paramList[, N:=1:.N]
...
for(i in 1:nrow(paramList)) {
  ...
  write.csv(cv_pred, paste0("model/xgboost_cv/ensemble/ensemble_", Version, "/cv_pred_", Version, ".csv"), row.names=FALSE)
  ...
  write.csv(val_pred, paste0("model/xgboost_cv/ensemble/ensemble_", Version, "/val_pred_", Version, ".csv"), row.names=FALSE)
  ...
  write.csv(paramList, paste0("model/xgboost_cv/ensemble/ensemble_", Version, "/xgboost_cv_ensemble_", Version, ".csv"), row.names=FALSE)
}
